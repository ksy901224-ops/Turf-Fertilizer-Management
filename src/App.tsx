...
import { ChatIcon, LogoutIcon, TrashIcon, ClipboardListIcon, PlusIcon, SparklesIcon, ChevronDownIcon } from './icons';
...
export default function TurfFertilizerApp() {
  ...
  const handleGetRecommendation = async () => {
    ...
    try {
      ...
      const prompt = `당신은 데이터 기반 잔디 관리 전문가입니다. 다음 시비 기록을 분석하여 향후 최적 시비 처방을 한국어로 작성해주세요.
      데이터: ${JSON.stringify(log.slice(0, 5))}`;
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <section className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                    <input type="date" value={date} onChange={e=>setDate(e.target.value)} className="p-3 border rounded-lg" />
                    <input type="number" value={applicationRate} onChange={e=>setApplicationRate(e.target.value)} placeholder="사용량" className="p-3 border rounded-lg" />
                </div>
            </div>
            <div className="flex gap-2 mb-4">
                {['그린', '티', '페어웨이'].map(t => (
                    <button key={t} onClick={() => setActiveLogTab(t as any)} className={`flex-1 py-2 rounded-lg font-bold text-sm ${activeLogTab === t ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{t}</button>
                ))}
            </div>
            <input type="number" value={logArea} onChange={e=>setLogArea(e.target.value)} placeholder={`${activeLogTab} 면적 (㎡)`} className="w-full p-3 border rounded-lg mb-4" />
        ...
        <section className="bg-gradient-to-br from-indigo-600 to-purple-700 p-8 rounded-2xl shadow-xl text-white">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <p className="text-indigo-100 text-sm max-w-md">시비 데이터를 바탕으로 현재 잔디 상태를 진단합니다.</p>
            <button 
              onClick={handleGetRecommendation}
              disabled={isLoadingAI}
              className="px-8 py-3 bg-white text-indigo-700 font-extrabold rounded-full hover:bg-indigo-50 shadow-lg flex items-center gap-2"
            >
              {isLoadingAI ? '분석 중...' : '맞춤 리포트 생성'}
            </button>
          </div>
          {aiResponse && (
            <div className="mt-6 p-6 bg-white/10 backdrop-blur-md rounded-xl border border-white/20 text-sm whitespace-pre-wrap leading-relaxed">
                {aiResponse}
            </div>
          )}
        </section>
        ...
        <section className="space-y-4">
          ...
          {log.length > 0 ? log.slice(0, 10).map(entry => (
            <div key={entry.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex justify-between items-center group">
              <div>
              ...
              <div className="flex items-center gap-4">
                 ...
                 <button onClick={() => { if(window.confirm('삭제?')) { const n = log.filter(l=>l.id!==entry.id); setLog(n); api.saveLog(user!, n); } }} className="p-2 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><TrashIcon /></button>
              </div>
            </div>
          )) : <div className="p-12 text-center bg-white rounded-xl border border-dashed text-slate-400">기록이 없습니다.</div>}
        </section>
